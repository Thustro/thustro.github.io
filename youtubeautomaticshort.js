...
// ------------------------
// FETCH LATEST LONGFORM VIDEO
// ------------------------
async function fetchLatestShortVideo(channelId) {
    const rssUrl = `https://www.youtube.com/feeds/videos.xml?channel_id=${channelId}`;
    const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(rssUrl)}`;

    const response = await fetch(proxyUrl);
    const data = await response.json();
    const xmlText = data.contents; // Extract XML from proxy response
    ...
    for (let entry of entries) {
        ...
        if (link && link.includes("/shorts/")) {
            ...
            const idMatch = link.match(/\/shorts\/([\w\-]+)/);
            if (idMatch) return idMatch[1];
        }
    }
    throw new Error("No short video found in RSS feed");
}
...
async function init() {
    try {
        videoId = await fetchLatestShortVideo(channelId);
        lastVideoId = videoId;
        ...
        // Refresh stats and chart every 5 seconds
...
async function checkForNewVideo() {
    try {
        const newVideoId = await fetchLatestShortVideo(channelId);
...
async function getthumbnail() {
    ...
    try {
        ...
        const nameObj = dataCarry.user.find(item => item.value === "name");
        const pfpObj = dataCarry.user.find(item => item.value === "pfp");
...
// ------------------------
// CHART - LIFETIME VIEWS PAST HOUR
// ------------------------
async function getData() {
    ...
    try {
        ...
        const viewsObj = data.counts.find(c => c.value === "views");
        if (!viewsObj || !viewsObj.count) return;

        const lifetimeViews = parseInt(viewsObj.count);
        const now = Date.now();

        // Store only last 1 hour of data
        apiData.push([now, lifetimeViews]);
        apiData = apiData.filter(([timestamp]) => now - timestamp <= 3600 * 1000);
