...
async function fetchLatestLongformVideo(channelId) {
    ...
    for (let entry of entries) {
        ...
        if (link && link.includes("watch?v=")) {
            const idMatch = link.match(/v=([\w\-]+)/);
            if (idMatch) return idMatch[1];
        }
    }
    throw new Error("No longform video found in RSS feed");
}
...
async function init() {
    try {
        videoId = await fetchLatestLongformVideo(channelId);
        lastVideoId = videoId;
...
async function checkForNewVideo() {
    try {
        const newVideoId = await fetchLatestLongformVideo(channelId);
